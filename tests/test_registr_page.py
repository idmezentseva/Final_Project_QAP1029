# python -m pytest -v --driver Chrome --driver-path Final_Project_QAP1029/chromedriver_win/chromedriver.exe tests/test_registr_page.py
import pytest
...
def test_page_left_registration(browser):
    """TК-11 Левая часть формы «Регистрация» содержит логотип и продуктовый слоган кабинета."""
    try:
        page_reg = RegistrPage(browser)
        assert page_reg.page_left_registration.text != ''
    except AssertionError:
        print('Элемент отсутствует в левой части формы')


def test_elements_of_registr(browser):
    """TК-12 Блок регистрации формы «Регистрация» содержит основные элементы:
...
def test_names_elements_of_registr(browser):
    """TК-13 Названия элементов блока регистрации формы «Регистрация» соответствуют Требованию."""
    try:
        page_reg = RegistrPage(browser)
...
def test_registr_by_valid_data(browser):
    """TК-14 Регистрация пользователя с валидными данными"""
    page_reg = RegistrPage(browser)
...
def test_registr_by_invalid_data(browser):
    """TК-15 Регистрация пользователя по email, который уже был использован ранее для регистрации"""
    page_reg = RegistrPage(browser)
    page_reg.first_name.send_keys(Settings.f_name)
    page_reg.first_name.clear()
    page_reg.last_name.send_keys(Settings.l_name)
...
def test_registr_and_redir_auth(browser):
    """TК-16 Регистрация пользователя по email, который уже был использован ранее для регистрации
...
def test_first_name_by_valid_data(browser, valid_first_name):
    """TК-17 Проверка поля ввода "Имя" формы «Регистрация» валидными данными:
...
def test_first_name_by_invalid_data(browser, invalid_first_name):
    """TК-18 Проверка поля ввода "Имя" формы «Регистрация» невалидными данными:
    пустое значение;
...
def test_last_name_by_valid_data(browser, valid_password):
    """TК-19 Проверка поля ввода "Пароль" формы «Регистрация» валидными данными:
...
def test_passw_registration_confirm_valid_data(browser):
    """TК-20 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» валидными данными
    (пароли совпадают)."""
    page_reg = RegistrPage(browser)
...
def test_passw_registration_confirm_invalid_data(browser):
    """TК-21 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» невалидными данными
    (пароли не совпадают)."""
    page_reg = RegistrPage(browser)
    page_reg.passw_registration.send_keys(Settings.passw1)
    page_reg.passw_registration.clear()
