# python -m pytest -v --driver Chrome --driver-path Final_Project_QAP1029/chromedriver_win/chromedriver.exe tests/test_auth_page.py
import pytest
...
def test_page_right(browser):
    """TК-01 В правой части формы «Авторизация» находится продуктовый слоган ЛК "Ростелеком ID"."""
    try:
...
def test_elements_of_auth(browser):
    """TК-02 Блок аутентификации формы «Авторизация» содержит основные элементы (Меню выбора типа аутентификации,
...
def test_menu_of_type_auth(browser):
    """TК-03 Меню выбора типа аутентификации содержит табы: 'Номер', 'Почта', 'Логин', 'Лицевой счёт'."""
    try:
        page = AuthPage(browser)
...
def test_menu_of_type_active_auth(browser):
    """TК-04 В Меню выбора типа аутентификации по умолчанию выбрана форма аутентификации по телефону."""
    page = AuthPage(browser)

    assert page.active_tub_phone.text == Settings.menu_of_type_auth[0]


def test_placeholder_name_of_user(browser):
    """TК-05 В форме ввода ('Номер', 'Почта', 'Логин', 'Лицевой счёт') плейсхолдер меняется в соответствии с
...
def test_forgot_password_link(browser):
    """TК-06 Проверка перехода по ссылке 'Забыл пороль'."""
    page = AuthPage(browser)
...
def test_register_link(browser):
    """TК-07 Проверка перехода по ссылке 'Зарегистрироваться'."""
    page = AuthPage(browser)
...
def test_auth_by_valid_email_pass(browser):
    """TК-08 Аутентификация пользователя с валидным email и паролем."""
    page = AuthPage(browser)
...
def test_auth_by_invalid_email(browser, incor_email, incor_passw):
    """TК-09, TК-10 Аутентификация пользователя с невалидным email и паролем:
